...
export const ReservationCalendar = ({ activities }: ReservationCalendarProps) => {
  ...
  const handleBooking = () => {
    if (!selectedActivity || !selectedDate || !selectedTime) {
      toast.error('Veuillez sélectionner une activité, une date et une heure');
      return;
    }
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 lg:p-8">
            ...
            >
              ...
              <div className="flex items-center gap-4 text-sm">
                ...
                <span className="font-semibold text-primary">
                  {formatPrice(activity.priceMAD)} / {t('common.per_person')}
                </span>
              </div>
            </button>
          ))}
      ...
      {selectedActivity && (
        <>
                ...
                {days.map((day, index) => (
                  <button
                    ...
                    className={`
                      ...
                      ${!day ? 'invisible' : ''}
                      ${isDatePast(day) ? 'text-muted/40 cursor-not-allowed' : 'hover:bg-secondary cursor-pointer'}
          ...
          {selectedDate && (
            <div className="mb-8">
              <h5 className="text-lg font-semibold mb-3">Sélectionnez l'heure</h5>
          ...
          {selectedTime && (
            <div className="mb-8">
              <h5 className="text-lg font-semibold mb-3">{t('calendar.participants')}</h5>
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setParticipants(Math.max(1, participants - 1))}
                  className="w-10 h-10 rounded-full border-2 border-border hover:border-primary transition-colors flex items-center justify-center"
                >
                  -
                </button>
          ...
          {selectedTime && (
            <div className="border-t border-border pt-6">
              ...
              <div className="text-sm text-muted space-y-1">
                <p>📅 {selectedDate.toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                <p>🕐 {selectedTime}</p>
                <p>👥 {participants} {participants === 1 ? 'participant' : 'participants'}</p>
              </div>
            </div>
          )}
        </>
